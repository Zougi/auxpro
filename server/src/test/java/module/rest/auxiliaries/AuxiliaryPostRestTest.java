...
public class AuxiliaryPostRestTest extends RestTestBase {
	...
	public void testI_sameName() throws Exception {
		...
		prepare("", userAdmin.getName(), userAdmin.getPassword()).post(Entity.entity(Mappers.DEFAULT.getMapper().writeValueAsString(bean), MediaType.APPLICATION_JSON));
		Response response = prepare("", userAdmin.getName(), userAdmin.getPassword()).post(Entity.entity(Mappers.DEFAULT.getMapper().writeValueAsString(bean2), MediaType.APPLICATION_JSON));
		AssertHelper.assertException(APException.USER_NAME_INUSE, response);
	}
	@Test
	public void testI_noName() throws Exception {
		bean.setName(null);
		Response response = prepare("", userAdmin.getName(), userAdmin.getPassword()).post(Entity.entity(Mappers.DEFAULT.getMapper().writeValueAsString(bean), MediaType.APPLICATION_JSON));
	...
	public void testV_asAdmin() throws Exception {
		AuxiliaryBean response = prepare("", userAdmin.getName(), userAdmin.getPassword()).post(Entity.entity(Mappers.DEFAULT.getMapper().writeValueAsString(bean), MediaType.APPLICATION_JSON), AuxiliaryBean.class);
		AssertHelper.assertCredentials(bean, response);
	}
	@Test
	public void testV_asGuest() throws Exception {
		Response response = prepare("", userGuest.getName(), userGuest.getPassword()).post(Entity.entity(Mappers.DEFAULT.getMapper().writeValueAsString(bean), MediaType.APPLICATION_JSON));
