...
public class UsersPostRestTest extends RestTestBase {
	...
	public void testI_postUsers_restrictedUser() throws Exception {
		...
		Response response = prepare("", userAux1.getName(), userAux1.getPassword()).post(Entity.entity(Mappers.DEFAULT.getMapper().writeValueAsString(bean), MediaType.APPLICATION_JSON));
	...
	public void testV_postUsers() throws Exception {
		UserBean userAdmin = TestData.getUserFromJson("users_admin.json");
		UserBean bean = TestData.getNextUser();
		UserBean response = prepare("", userAdmin.getName(), userAdmin.getPassword()).post(Entity.entity(Mappers.DEFAULT.getMapper().writeValueAsString(bean), MediaType.APPLICATION_JSON), UserBean.class);
